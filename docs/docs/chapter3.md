...
**In plain English:** Your data pipelines will encounter errors—it's not a matter of "if" but "when." These patterns help you handle the inevitable gracefully.
...
Data is dynamic, and your expectations from today won't remain the same throughout the lifecycle. You're processing data generated by others, directly inheriting their issues—unreliable networks causing late delivery, temporary crashes causing retried deliveries, and subsequent duplicates.

<CardGrid
  columns={3}
  cards={[
    { title: "Data Quality", icon: "📊", color: colors.red, items: ["Unprocessable records", "Duplicates", "Late arrivals"] },
    { title: "System Failures", icon: "⚠️", color: colors.orange, items: ["Hardware issues", "Network problems", "Job crashes"] },
    { title: "Human Errors", icon: "👤", color: colors.purple, items: ["Buggy filters", "Wrong configurations", "Missing data"] }
  ]}
/>
...
> **Insight**
>
> **Transient vs. Nontransient Errors:** Transient errors are temporary and will recover automatically (like brief database unavailability). Nontransient errors—also known as "poison pill" messages—are fatal issues requiring manual intervention.

### 2.1. Pattern: Dead-Letter

**In plain English:** Instead of crashing when you hit a bad record, set it aside for later investigation and keep processing the good ones—like a postal service returning undeliverable mail to a "dead letter" office.
...
<DiagramContainer title="Dead-Letter Pattern Architecture">
  <Column gap="lg">
    <Row gap="md" align="center">
      <Box color={colors.blue} icon="📥">Data Source</Box>
      <Arrow direction="right" />
      <Box color={colors.purple} icon="⚙️">Processing Job</Box>
    </Row>
    <Row gap="lg" align="center">
      <Column gap="sm" align="center">
        <Arrow direction="down" label="valid" />
        <Box color={colors.green} icon="✅">Main Output</Box>
      </Column>
      <Column gap="sm" align="center">
        <Arrow direction="down" label="errors" />
        <Box color={colors.red} icon="❌">Dead-Letter Store</Box>
      </Column>
    </Row>
    <Row gap="md" align="center">
      <Box color={colors.slate} icon="📊">Monitoring</Box>
      <Arrow direction="right" label="optional" />
      <Box color={colors.orange} icon="🔄">Replay Pipeline</Box>
    </Row>
  </Column>
</DiagramContainer>
...
| **Monitoring** | Key success factor—know when errors spike |
| **Performance** | Writing unprocessed records incurs execution cost |

Good candidates include cloud object stores and streaming brokers—highly available, fast, and easy to monitor.
...
<DiagramContainer title="Snowball Backfilling Effect">
  <Column gap="md">
    <Row gap="md" align="center">
      <Box color={colors.red} icon="🔄">Pipeline 1 Backfill</Box>
    </Row>
    <Arrow direction="down" label="triggers" />
    <Row gap="md" align="center">
      <Box color={colors.orange} icon="🔄">Consumer A Backfill</Box>
      <Box color={colors.orange} icon="🔄">Consumer B Backfill</Box>
    </Row>
    <Arrow direction="down" label="triggers" />
    <Row gap="md" align="center">
      <Box color={colors.purple} icon="🔄">Consumer C</Box>
      <Box color={colors.purple} icon="🔄">Consumer D</Box>
      <Box color={colors.purple} icon="🔄">Consumer E</Box>
    </Row>
  </Column>
</DiagramContainer>
...
| **Hidden failures** | Pattern can mask fatal issues—complete with alerting for high error rates |
...
**In plain English:** Like a bouncer with a guest list who remembers who's already entered the party—within a certain time window, they won't let the same person in twice.

**In technical terms:** The Windowed Deduplicator pattern eliminates duplicate records by tracking unique identifiers within a defined scope—the entire dataset for batch jobs, or time-based windows for streaming jobs.
...
<DiagramContainer title="Windowed Deduplicator for Streaming">
  <Row gap="md" align="center">
    <Box color={colors.blue} icon="📥">Input Record</Box>
    <Arrow direction="right" />
    <Box color={colors.purple} icon="🔑">Extract Key</Box>
    <Arrow direction="right" />
    <Box color={colors.orange} icon="💾">Check State Store</Box>
  </Row>
</DiagramContainer>
  ...
  <Column gap="md">
    <Row gap="lg" align="center">
      <Group title="Key Already Seen?" color={colors.orange}>
        <Row gap="md">
          <Column gap="sm" align="center">
            <Box color={colors.green} icon="✅" size="sm">No</Box>
            <Arrow direction="down" />
            <Box color={colors.green} icon="📤" size="sm">Process & Store Key</Box>
          </Column>
          <Column gap="sm" align="center">
            <Box color={colors.red} icon="❌" size="sm">Yes</Box>
            <Arrow direction="down" />
            <Box color={colors.slate} icon="🗑️" size="sm">Drop Record</Box>
          </Column>
        </Row>
      </Group>
    </Row>
...
<CardGrid
  columns={3}
  cards={[
    { title: "Local", icon: "💨", color: colors.green, items: ["Fastest performance", "State in memory only", "Lost on failure"] },
    { title: "Local + Fault-Tolerant", icon: "⚖️", color: colors.blue, items: ["Fast memory access", "Persisted to remote storage", "Trade-off: speed vs. consistency"] },
    { title: "Remote", icon: "🔒", color: colors.purple, items: ["Native fault tolerance", "Higher latency", "Additional cost"] }
  ]}
/>
...
| **Not exactly-once delivery** | Correct deduplication doesn't guarantee exactly-once delivery—consider idempotency patterns |
...
**In plain English:** Like setting a deadline for party RSVPs—anyone who responds after the cutoff is marked as "late," and you decide how to handle them separately.
...
> **Insight**
>
> **Event Time vs. Processing Time:** Event time indicates when an action happened; processing time indicates when the pipeline processes it. Processing time is never late—only event time can be.
...
<ProcessFlow
  ...
  steps={[
    { title: "Define Event Time", description: "Select time attribute to track", icon: "⏰", color: colors.blue },
    { title: "Aggregate Per Partition", description: "Use MAX for monotonicity", icon: "📊", color: colors.purple },
    { title: "Global Aggregation", description: "MIN or MAX across partitions", icon: "🌐", color: colors.green },
    { title: "Apply Lateness", description: "MAX(event_time) - lateness", icon: "⚡", color: colors.orange }
  ]}
/>
...
|-------------|-----------------|---------------------|------------------|-----------------|
| 10:00, 10:05, 10:06 | - | MAX(10:06) − 30' = 9:36 | 9:36 | - |
| 9:20, 9:31, 10:07 | 9:36 | MAX(10:07) − 30' = 9:37 | 9:37 | 9:20, 9:31 |
...
|------------|---------------------|------------------|-----------------|
| 03:15 | epoch → 02:15 | [03:10-03:20] | [] |
| 03:00 | 02:15 → 02:15 | [03:00-03:10, 03:10-03:20] | [] |
| 01:50 | 02:15 → 02:15 | [03:00-03:10, 03:10-03:20] | [] (01:50 ignored) |
...
**In plain English:** Like checking the mailbox for the past two weeks every day—regardless of whether anything actually arrived late during that period.
...
<DiagramContainer title="Static Lookback Window">
  <Row gap="md" align="center">
    <Box color={colors.slate} icon="📅">Dec 17</Box>
    <Box color={colors.slate} icon="📅">Dec 18</Box>
    <Box color={colors.blue} icon="📅">...</Box>
    <Box color={colors.orange} icon="📅">Dec 30</Box>
    <Box color={colors.green} icon="⭐">Dec 31 (Current)</Box>
  </Row>
</DiagramContainer>
...
|---------------|---------------------|
| **Stateful** | Sequential (late data first)—need valid history for current data |
...
<DiagramContainer title="State Table for Dynamic Late Data Integration">
  <Column gap="md">
    <Box color={colors.blue} icon="📋" size="lg">State Table</Box>
    <Row gap="lg" align="center">
      <Column gap="sm" align="center">
...
> **Insight**
>
> **Entity-Level Optimization:** If you can isolate entities impacted by late data, you don't need to backfill entire partitions—just overwrite data for affected entities. This optimizes resources but is harder to implement.
...
Data engineering errors aren't always technical failures—human mistakes like incorrect filter implementations can expose partial or bad data to end users.

### 5.1. Pattern: Filter Interceptor

**In plain English:** Like a detailed receipt showing exactly which items were rejected at airport security and why—instead of just knowing "some items were confiscated."
...
<DiagramContainer title="Filter Interceptor Pattern">
  <Row gap="md" align="center">
    ...
    <Arrow direction="right" />
    <Column gap="sm">
      <Box color={colors.purple} icon="🔍" size="sm">Filter 1 + Counter</Box>
      <Box color={colors.purple} icon="🔍" size="sm">Filter 2 + Counter</Box>
      <Box color={colors.purple} icon="🔍" size="sm">Filter N + Counter</Box>
    </Column>
    <Arrow direction="right" />
    <Box color={colors.green} icon="📤">Valid / Filtered</Box>
  </Row>
</DiagramContainer>
...
> **Insight**
>
> **Right Tool for the Job:** If programmatic API is better for your needs, use it—even if you've been writing SQL exclusively. The Filter Interceptor is a case where declarative languages are less powerful.
...
**In plain English:** Like a video game autosave—when you crash, you don't start from the very beginning; you resume from the last saved checkpoint.
  ...
  <Row gap="lg" align="center">
    <Group title="Framework-Based" color={colors.blue}>
      <Column gap="sm">
        ...
        <Box color={colors.slate} size="sm" icon="💾">Object Store</Box>
      </Column>
    </Group>
    <Group title="Data Store-Based" color={colors.green}>
      <Column gap="sm">
        ...
        <Box color={colors.slate} size="sm" icon="💾">Kafka Topic / DynamoDB</Box>
      </Column>
    </Group>
  </Row>
</DiagramContainer>
...
<CardGrid
  columns={3}
  cards={[
    { title: "At Most Once", icon: "1️⃣", color: colors.orange, items: ["Checkpoint before processing", "May lose data on failure"] },
    { title: "At Least Once", icon: "➕", color: colors.blue, items: ["Checkpoint after processing", "May create duplicates on retry"] },
    { title: "Exactly Once", icon: "✓", color: colors.green, items: ["Requires idempotency patterns", "Checkpointing alone insufficient"] }
  ]}
/>
...
<CardGrid
  columns={2}
  cards={[
    { title: "Dead-Letter", icon: "📬", color: colors.red, items: ["Isolate unprocessable records", "Keep pipeline running", "Enable post-mortem analysis"] },
    { title: "Windowed Deduplicator", icon: "🔑", color: colors.blue, items: ["Eliminate duplicates in scope", "Batch: full dataset", "Streaming: time windows"] },
    { title: "Late Data Detector", icon: "⏰", color: colors.orange, items: ["Watermark-based detection", "Event time vs. processing time", "Monotonically increasing"] },
    { title: "Late Data Integrators", icon: "🔄", color: colors.purple, items: ["Static: fixed lookback window", "Dynamic: only changed partitions", "Watch for snowball effects"] },
    { title: "Filter Interceptor", icon: "🔍", color: colors.green, items: ["Track filter statistics", "Detect aggressive filtering", "Debug data volume changes"] },
    { title: "Checkpointer", icon: "💾", color: colors.cyan, items: ["Persist progress to storage", "Enable failure recovery", "Not true exactly-once alone"] }
  ]}
/>
